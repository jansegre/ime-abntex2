...
# filename.transcript.mk for each command that was run, including
...
define transcript
$(if $(WRITE_TRANSCRIPT), \
	$(ECHO) "Running $1 ($$run_reason)" >> $2.transcript.mk; \
	export run_reason="", \
	$(sh_true))
endef
...
-e 'd' \
'$1' > "$1.$$$$.sed.mk"; \
$(SED) -f "$1.$$$$.sed.mk" '$1' > "$1.$$$$.mk"; \
$(SED) \
-e '/^\\relax/d' \
-e '/^\\bibcite/d' \
-e 's/^\(\\newlabel{[^}]\{1,\}}\).*/\1/' \
"$1.$$$$.mk" | $(SORT) > '$2'; \
$(call remove-temporary-files,$1.$$$$.mk $1.$$$$.sed.mk)
endef
...
#
define convert-gpi
$(ECHO) 'set terminal $(call gpi-terminal,$1,$2,$3)' > $1head.mk; \
$(ECHO) 'set output "$2"' >> $1head.mk; \
$(if $(gpi_global),$(CAT) $(gpi_global) >> $1head.mk;,) \
fnames='$1head.mk $1';\
$(if $(gpi_sed),\
	$(SED) -f '$(gpi_sed)' $$fnames > $1.temp.mk; \
	fnames=$1.temp.mk;,\
) \
success=1; \
if ! $(GNUPLOT) $$fnames 2>$1.log; then \
	...
	if ! $(call gpi-embed-pdf-fonts,$2,$2.embed.tmp.mk); then \
		success=0; \
	else \
		$(call move-if-exists,$2.embed.tmp.mk,$2); \
	fi; \
fi; \
$(if $(gpi_sed),$(call remove-temporary-files,$1.temp.mk);,) \
$(call remove-temporary-files,$1head.mk); \
[ "$$success" = "1" ] && $(sh_true) || $(sh_false);
endef
...
%.pdf: %.ps %.embed.mk
	...
	$(QUIET)$(call make-pdf,$<,$@.temp,$@.log,$*.embed.mk); \
	if [ x"$$?" = x"0" ]; then \
	    $(if $(VERBOSE),$(CAT) $@.log,:); \
	    $(RM) -f '$@'; \
	    $(MV) '$@.temp' '$@'; \
...
%.ps: %.dvi %.paper.mk %.beamer.mk
	$(QUIET)$(call echo-build,$<,$@)
	$(QUIET)$(call make-ps,$<,$@.temp,$@.log,\
			$(strip $(shell $(CAT) $*.paper.mk)),\
			$(strip $(shell $(CAT) $*.beamer.mk))); \
	if [ x"$$?" = x"0" ]; then \
	    $(if $(VERBOSE),$(CAT) $@.log,:); \
	    $(RM) -f '$@'; \
	    $(MV) '$@.temp' '$@'; \
...
#
#		Note that if the .auxtarget.mk file doesn't exist, this means
#		that we are doing a clean build, so it doesn't figure into the
#		test for running again.
#
#		compare against .auxtarget.mk
#
...
#
#	If we do NOT need to run latex here, then we move the $@.1st.mk file
...
%.$(build_target_extension): %.bbl %.aux %.$(build_target_extension).1st.mk
	$(QUIET)\
	...
	$(call make-auxtarget-file,$*.aux.mk,$*.auxtarget.cookie); \
	run=0; \
	for i in 1; do \
		...
		if $(call \
		test-exists-and-different,$*.auxtarget.cookie,$*.auxtarget.mk);\
		then \
			$(call set-run-reason,$*.auxtarget.cookie differs from $*.auxtarget.mk); \
			run=1; \
			break; \
		fi; \
		...
		if $(call test-not-exists,$@.1st.mk); then \
			$(call set-run-reason,$@.1st.mk does not exist); \
			run=1; \
			break; \
		fi; \
	done; \
	$(call remove-temporary-files,$*.bbl.cookie $*.run.cookie); \
	$(MV) $*.auxtarget.cookie $*.auxtarget.mk; \
	if [ x"$$run" = x"1" ]; then \
		$(call remove-files,$@.1st.mk); \
		for i in 2 3 4 5; do \
...
# dependency creation also produces the %.auxbbl.mk file.  BibTeX is a bit
# finicky about what you call the actual files, but we can rest assured that if
# a .auxbbl.mk file exists, then the .aux file does, as well.  The
# .auxbbl.mk file is a cookie indicating whether the .bbl needs to be
...
%.eps %.mps %.mpx %.log: %.mp
	...
	$(QUIET)$(call move-if-exists,$*.log,$*.log.mk)
	$(QUIET)$(call move-if-exists,$*.mpx,$*.mpx.mk)
...
#	%.d
#	%.aux.mk
#	%.(pdf|dvi).1st.mk (the .pdf or .dvi output file, moved)
#
#	Steps:
#
#	Run latex
...
#	not creating a .1st.mk file..
#
%.$(build_target_extension).1st.mk %.d %.aux %.aux.mk %.fls: %.tex
	$(QUIET)$(call echo-build,$<,$*.d $*.$(build_target_extension).1st.mk,$(RESTARTS)-1)
	$(QUIET)\
	$(call set-run-reason,need to build .d and .$(build_target_extension).1st.mk); \
	$(call run-latex,$*,-recorder) || $(sh_true); \
	$(CP) '$*.log' '$*.$(RESTARTS)-1.log'; \
	$(call die-on-import-sty,$*.log); \
	$(call die-on-dot2tex,$*.log); \
	$(call die-on-no-aux,$*); \
	$(call flatten-aux,$*.aux,$*.aux.mk); \
	$(ECHO) "# vim: ft=make" > $*.d; \
	$(ECHO) ".PHONY: $*._graphics" >> $*.d; \
	$(call get-inputs,$*.fls,$(addprefix $*.,aux aux.mk d $(build_target_extension))) >> $*.d; \
	$(call get-format,$<,$(addprefix $*.,fls aux aux.mk d $(build_target_extension) $(build_target_extension).1st.mk)) >> $*.d; \
	$(call get-missing-inputs,$*.log,$(addprefix $*.,aux aux.mk d $(build_target_extension))) >> $*.d; \
	$(ECHO) ".SECONDEXPANSION:" >> $*.d; \
	$(call get-graphics,$*) >> $*.d; \
	$(call get-log-index,$*,$(addprefix $*.,aux aux.mk)) >> $*.d; \
	$(call get-bibs,$*.aux.mk,$(addprefix $*.,bbl aux aux.mk)) >> $*.d; \
	$(EGREP) -q "# MISSING stem" $*.d && $(SLEEP) 1 && $(RM) $*.pdf; \
	$(EGREP) -q "# MISSING format" $*.d && $(RM) $*.pdf; \
	$(call move-if-exists,$*.$(build_target_extension),$*.$(build_target_extension).1st.mk); \
	for s in toc out lot lof lol nav; do \
		if [ -e "$*.$$s" ]; then \
			if ! $(DIFF) -q $*.$$s $*.$$s.mk >/dev/null 2>&1; then \
				$(TOUCH) $*.run.cookie; \
			fi; \
			$(CP) $*.$$s $*.$$s.mk; \
		fi; \
...
.SECONDARY: $(addsuffix .auxbbl.mk,$(stems_ssg))
%.auxbbl.mk: %.aux.mk
	$(QUIET)\
...
.SECONDARY: $(addsuffix .specials.mk,$(all_stems_ss))
%.beamer.mk: %.specials.mk
...
.PHONY: clean-deps
clean-deps:
	$(QUIET)$(call clean-files,$(all_d_targets) *.mk *.mk.temp *.cookie)
...
#
#    KEEP_TEMP:
#        When set, this allows .mk and other temporary files to stick around
